...
abstract class Student {
    String id;
    TreeMap<Integer, List<Integer>> gradesByTerm; //Integer go pretstavuva smestarot, List<Integer> ocenki vo toj semestar
    TreeSet<String> coursesAttended; //site kursevi za eden student
    int coursesPassed;
    ...
    public void addGrade(int term, String courseName, int grade) throws OperationNotAllowedException {
        if (!gradesByTerm.containsKey(term)) //ako ne go sodrzi semestarot frli isklucok
            throw new OperationNotAllowedException(String.format("Term %d is not possible for student with ID %s", term, id));

        List<Integer> gradesForTerm = gradesByTerm.get(term); //lista so site ocenki od daden semestar
        if (gradesForTerm.size() == 3) //ne moze da se dodade uste enda ocenka
            throw new OperationNotAllowedException(String.format("Student %s already has 3 grades in term %d", id, term));

        gradesForTerm.add(grade); //se dodava ocenkata
        coursesAttended.add(courseName); //se dodava predmetot
        coursesPassed++; //zgloemi go brojot na polozeni predmeti
    }
    ...
    public boolean hasGraduated() {
        int term = gradesByTerm.size(); //vrakja broj na (K,V) parovi
        return term * 3 == coursesPassed;
    }
    //prosek na student
    public double calculateAverage() {
        //flatMap za sekoja lista povikuva stream
        return gradesByTerm.values()
                .stream()
                .flatMap(list -> list.stream())
                .mapToInt(i -> i)
                .average()
                .orElse(5.0); //ako nema ocenka vrati 5.0
    }
    //prosek za semestar
    public double calculateAverageForTerm(int term) {
    ...
    public String getDetailedReport() {
        ...
        gradesByTerm.keySet().forEach(term -> {
            sb.append(getDetailedReportForTerm(term)).append("\n");
        });
        //ili
        //sb.append(gradesByTerm.keySet().stream().map(term -> getDetailedReportForTerm(term)).collect(Collectors.joining("\n")));
        sb.append(String.format("Average grade: %.2f\n", calculateAverage()));
        sb.append(String.format("Courses attended: %s", coursesAttended.stream().collect(Collectors.joining(","))));
        return sb.toString();
    }
...
class Student3 extends Student {
    public Student3(String id) {
        super(id);
        for (int i = 1; i <= 6; i++) {
            gradesByTerm.put(i, new ArrayList<>()); //za sekoj semestar 1-6 stavi prazna lista
        }
    }
...
class Student4 extends Student {
    public Student4(String id) {
        super(id);
        for (int i = 1; i <= 8; i++) {
            gradesByTerm.put(i, new ArrayList<>()); //za sekoj semestar 1-8 stavi prazna lista
        }
    }
...
class Faculty {
    ...
    void addGradeToStudent(String studentId, int term, String courseName, int grade) throws OperationNotAllowedException {
        ...
        if (s.hasGraduated()) {
            //sbForLogs.append(s.getLog()).append("\n");
            logs.add(s.getLog());
